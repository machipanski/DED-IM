...
class System_Paths:
    """Maintains the organization of paths within the program's folder to avoid loading unprocessed items."""
    ...
    def load_island_paths_hdf5(self, layer_name: str, island: Island) -> None:
        ...
        if etr_group:
            island.external_tree_route = Path(
                ...
                jumps=list(etr_group.get("jumps")),
        ...
        if twtr_group:
            island.thinwalls_tree_route = Path(
                ...
                jumps=list(twtr_group.get("jumps")),
                img=self.load_img_hdf5(
                    f"/{layer_name}/{island.name}/thinwalls_tree_route", "img"
                ),
            )
        ...
        if isltr_group:
            island.island_route = Path(
                ...
                jumps=list(isltr_group.get("jumps")),
                img=self.load_img_hdf5(
    ...
    def load_layers_hdf5(self) -> List[Layer]:
        ...
        for key, item in f.items():
            if key == "folders_structure":
                for name_atributo, valor_atributo in f[key].attrs.items():
                    setattr(self, name_atributo, valor_atributo)
            else:
    ...
    def save_layers(self, save_name, layers_list: List[Layer]):
        os.chdir(self.output)
        try:
            f = h5py.File(save_name + ".hdf5", "r")
        except:
            last_part = save_name.rsplit("/", 1)[-1]
            self.create_hdf5_file(last_part)
        ...
        for isl in islands:
            ...
            if len(isl.thin_walls.regions) > 0:
                ...
                for reg in isl.thin_walls.regions:
                    ...
                    self.save_img_hdf5(tw_group_path, f"line1", reg.contour_elements[0])
                    self.save_img_hdf5(tw_group_path, f"line2", reg.contour_elements[1])
                    self.save_img_hdf5(
                        tw_group_path, f"linebaixo", reg.contour_elements[3]
                    )
                    self.save_img_hdf5(
                        tw_group_path, f"linetopo", reg.contour_elements[2]
                    )
        return
        ...
        for isl in islands:
            if np.sum(isl.rest_of_picture_f3) > 0:
                ...
                for reg in isl.bridges.zigzag_bridges:
                    ...
                    self.save_img_hdf5(path_region, f"img", reg.img)
                    self.save_img_hdf5(path_region, f"origin", reg.origin)
                    self.delete_item_hdf5(path_region + f"/contour")
                    self.save_img_hdf5(path_region, f"contour", reg.contour)
                    self.save_props_hdf5(path_region, reg.__dict__)
    ...
    def save_external_routes_hdf5(self, layer_name, islands: List[Island]):
        for isl in islands:
            ...
            self.save_seq_hdf5(element_path, "jumps", isl.external_tree_route.jumps)
    ...
    def save_thinwall_final_routes_hdf5(self, layer_name, islands: List[Island]):
        for isl in islands:
            ...
            self.save_seq_hdf5(element_path, "jumps", isl.thinwalls_tree_route.jumps)
            self.save_props_hdf5(f"/{layer_name}/{isl.name}", isl.__dict__)
            ...
            lista_de_programas.append(
                {
                    ...
                    "voltage": prog["voltage"],
                    "on_pause": prog["on_pause"],
                    "off_pause": prog["off_pause"],
                }
            )
        configs = {
            "file": self.file,
        ...
        self.lista_programas.append(
            (
                WeldingProgram(
                    ...
                    on_pause=float(on_pause),
                    off_pause=float(off_pause),
                )
            ).__dict__
        )
        self.updateConfigs()
