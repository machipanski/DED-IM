...
class Area:
    """Each closed contour in a level generated by the Offsets"""

    def __init__(self, name, img, origem, loops_inside):
        self.name = name
        self.img = img
...
class Loop:
    ...
    # def __init__(self, name, img, origem, trail, **kwargs):
...
class Level:
    """Each sequential element generated by the Offset operation on an image"""

    def __init__(self, img: str, name, pai, area, path):
        self.name = name
        self.parent = pai
        self.img = img
    ...
    def create_level(
        ...
        if first:
            ...
            new_lvl_area = mt.erosion(new_lvl_img, kernel_img=mask_trail)
        return Level(new_lvl_img, name_filho, self.name, new_lvl_area, path)
    ...
    def calculate_lost_area(self, result):
        """Calculates the difference between what was covered and the ideal area
        of each Loop."""
    ...
    def divide_composed_areas(self, area, path_radius):
        """When an area has more than one loop, what is the ideal area for EACH LOOP?"""
...
class OffsetRegions:
    ...
    def create_influence_regions(self, base_frame):
        """Performs a scan from the inside out on the Levels of the Layer. If the external contour is inside another
        in the previous level, they are in the same group. If there are two or none, then it is a new group.
        After that, it does the same from the outside in with the internal contours of the holes.
        """
        influence_regions = []
        ...
        """Elimination process for each image. The final region is the image that completely contains the others."""
        ...
        """At the end, remove the innermost regions from the outermost ones"""
        pare = 0
        while pare == 0:
            n_alteracoes = 0
            to_remove = []
    ...
    def label_offset_regions(self, sequence, counter_start, order):
        """Separates a region for each loop while traversing the layers."""
            ...
            for loop_cam_atual in loop_group:
                ...
                if level_number == sequence[0]:
                    ...
                    loop_cam_atual.region = (
                        region_counter  # e dá o name dela. essa será uma seed
                    )
                else:
                    if order == 1:
    ...
    def make_valid_loops(self, base_frame):
        """Creates a matrix with all the routes in each Level summed for the Layer."""
    ...
    def sum_areas(self, levels, influence_regions, interval, outer_inner, base_frame):
        """For each region number, traverses all the layers looking for loops
        with that number. When found, performs an OR to sum the internal areas to the total region.
        """
        loop_group = []
        for i in interval:
            loops_inside = []
...
class Region:
    ...
    def __init__(self, name, img, loops):
        ...
        self.limit_coords = (
            []
        )  # coordinates of the points where monotonic regions are separated
        self.center_coords = []  # coordinates of the geometric center of each contour
        self.area_contour = []  # contour of each area
        self.area_contour_img = []
        self.internal_area = []  # result of filling the interior of each contour
        self.hierarchy = (
            0  # hierarchy of contours, which are internal and which are external
        )
        self.parallel_points = []
        self.route = []
        self.trail = []
    ...
    def out_area_inner_contour(self, base_frame):
        ...
        maior_soma = 0
        index_maior_contour_interno = 0
        for i in np.arange(0, len(this_contours)):
            soma = len(this_contours[i])
            if soma > maior_soma and this_hierarchy[0][i][2] == -1:
                maior_soma = soma
                index_maior_contour_interno = i
        area_internal_contour = this_contours[index_maior_contour_interno]
        area_internal_contour_img = drawContours(
            np.zeros(base_frame), this_contours, index_maior_contour_interno, 1
        )
        return area_internal_contour, area_internal_contour_img

    def make_parallel_points(
        ...
        for region in regions:
            if region.name != self.name:
                self.parallel_points.append(ColecaoDeCoords(self.name, region.name))
                ...
                for i in np.arange(0, len(destiny_points)):
                    ...
                    if (
                        ...
                        self.parallel_points[-1].lista_a.append(destiny_points[i])
                        line = it.draw_line(
                            np.zeros_like(prohibited_areas),
                            self.limmit_coords[0],
                            destiny_points[i],
                        )
                        line = mt.dilation(line, kernel_size=path_radius * 2 + 2)
                        if np.sum(np.logical_and(line, prohibited_areas)) > 0:
                            multiplier = 100
                        self.parallel_points[-1].dist_a.append(
                    ...
                    if (
                        ...
                        self.parallel_points[-1].lista_b.append(destiny_points[i])
                        line = it.draw_line(
                            np.zeros_like(prohibited_areas),
                        ...
                        if np.sum(np.logical_and(line, prohibited_areas)) > 0:
                            multiplier = 100
                        self.parallel_points[-1].dist_b.append(
                    ...
                    if (
                        ...
                        self.parallel_points[-1].lista_c.append(destiny_points[i])
                        line = it.draw_line(
                            np.zeros_like(prohibited_areas),
                        ...
                        if np.sum(np.logical_and(line, prohibited_areas)) > 0:
                            multiplier = 100
                        self.parallel_points[-1].dist_c.append(
                    ...
                    if (
                        ...
                        self.parallel_points[-1].lista_d.append(destiny_points[i])
                        line = it.draw_line(
                            np.zeros_like(prohibited_areas),
                        ...
                        if np.sum(np.logical_and(line, prohibited_areas)) > 0:
                            multiplier = 100
                        self.parallel_points[-1].dist_d.append(
